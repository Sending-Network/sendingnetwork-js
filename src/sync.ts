 ...
 * of SendingNetworkClient props (e.g. http). Given we want to support WebSockets as
...
import { IHiSeasPrivacyCapability, IStoredClientOpts, SendingNetworkClient, PendingEventOrdering } from "./client";
...
import { SendingNetworkError } from "./http-api";
 ...
 * Construct an entity which is able to sync with a node.
 * @constructor
 * @param {SendingNetworkClient} client The sendingnetwork client instance to use.
...
export class SyncApi {
     ...
     * @experimental
     */
    public partitionThreadedEvents(events: SendingNetworkEvent[]): [SendingNetworkEvent[], SendingNetworkEvent[]] {
        if (this.opts.experimentalThreadSupport) {
            return events.reduce((memo, event: SendingNetworkEvent) => {
                ...
                if (!shouldLiveInThreadTimeline) {
                    ...
                    const parentEvent = room?.findEventById(parentEventId) || events.find((mxEv: SendingNetworkEvent) => {
    ...
    private shouldAbortSync(error: SendingNetworkError): boolean {
    ...
    public sync(): void {
        ...
        const getFilter = async () => {
            ...
            await savedSyncPromise;
            await this.enhancePrivacyCheck();
    ...
    private async processSyncResponse(
        ...
        if (this.opts.crypto && data["org.sendingnetwork.msc2732.device_unused_fallback_key_types"]) {
            // The presence of device_unused_fallback_key_types indicates that the
            // server supports fallback keys. If there's no unused
            // signed_curve25519 fallback key we need a new one.
            const unusedFallbackKeys = data["org.sendingnetwork.msc2732.device_unused_fallback_key_types"];
    ...
    private enhancePrivacyForRoom(privacy: IHiSeasPrivacyCapability, room: Room) {
        if (!privacy || !room || room.privacyEnhanced) {
            return;
        }
        const stateEvents: SendingNetworkEvent[] = [];
        const liveEvents: SendingNetworkEvent[] = [];
        const now = (new Date()).getTime();
        if (privacy.force_e2e_encryption && this.client.isRoomEncrypted(room.roomId)) {
            const encryptionEvent: SendingNetworkEvent = room.currentState.events.get(EventType.RoomEncryption).get("");
            if (encryptionEvent
                && !room.getLiveTimeline().getEvents().find((e => e.event.type == EventType.RoomEncryption))) {
                const event = new SendingNetworkEvent({
                    ...encryptionEvent.event,
                    type: EventType.RoomEncryptionHint,
        ...
        if (privacy.messages_retention_duration > 0) {
            // Simulate retention event
            const createEvent: SendingNetworkEvent = room.currentState.events.get(EventType.RoomCreate).get("");
            const sender= createEvent.event.sender;
            const event: SendingNetworkEvent = new SendingNetworkEvent({
                content: { max_lifetime: privacy.messages_retention_duration, mock: true },
                origin_server_ts: now,
                state_key: "",
                type: EventType.RoomRetention,
                room_id: room.roomId,
                sender: sender,
                event_id: "$" + Math.random() + "-" + Math.random(),
            });
        ...
        this.processRoomEvents(room, stateEvents, liveEvents);
        if (stateEvents.length > 0) {
            this.client.emit("event", stateEvents[0]);
        }
        room.privacyEnhanced = true;
    }

    private async enhancePrivacyCheck() {
        const rooms = this.client.getRooms();
        if (rooms.length <= 0) {
            debuglog("No room(s) loaded, will check later.");
            return;
        }
        const unhandledRooms = rooms.filter(r => !r.privacyEnhanced);
        if (unhandledRooms.length <= 0) {
            debuglog("All room(s) already have privacy enhanced events.");
            return;
        }
        const caps = await this.client.getCapabilities();
        const privacy = caps["im.hiseas.privacy"];
        debuglog(`Perform privacy enhancement check on ${rooms.length} rooms: `, privacy);
        for (const i in rooms) {
            const room = rooms[i];
            this.enhancePrivacyForRoom(privacy, room);
        }
    }

    /**
     * Make a dummy call to /_api/client/versions, to see if the HS is
     * reachable.
     *
    ...
    /**
     * @param {Room} room
     * @param {SendingNetworkEvent[]} stateEventList A list of state events. This is the state
     * at the *START* of the timeline list if it is supplied.
     * @param {SendingNetworkEvent[]} [timelineEventList] A list of timeline events. Lower index
     * @param {boolean} fromCache whether the sync response came from cache
     * is earlier in time. Higher index is later.
     */
    private processRoomEvents(
        room: Room,
        stateEventList: SendingNetworkEvent[],
     ...
     * @experimental
     */
    private processThreadEvents(room: Room, threadedEvents: SendingNetworkEvent[]): void {
        threadedEvents
    ...
    // extractRelatedEvents(event: SendingNetworkEvent, events: SendingNetworkEvent[], relatedEvents: SendingNetworkEvent[] = []): SendingNetworkEvent[] {
     ...
     * is earlier in time. Higher index is later.
     */
    private processEventsForNotifs(room: Room, timelineEventList: SendingNetworkEvent[]): void {
...
function createNewUser(client: SendingNetworkClient, userId: string): User {
    const user = new User(userId);
    client.reEmitter.reEmit(user, [
        "User.avatarUrl", "User.displayName", "User.presence", "User.wallet",
