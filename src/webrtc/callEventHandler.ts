...
import { createNewSendingNetworkCall, SendingNetworkCall, CallErrorCode, CallState, CallDirection } from './call';
...
export class CallEventHandler {
    ...
    calls: Map<string, SendingNetworkCall>;
    callEventBuffer: SendingNetworkEvent[];
    candidateEventsByCall: Map<string, Array<SendingNetworkEvent>>;

    constructor(client: SendingNetworkClient) {
        this.client = client;
        this.calls = new Map<string, SendingNetworkCall>();
        ...
        this.candidateEventsByCall = new Map<string, Array<SendingNetworkEvent>>();
    }
    ...
    private onRoomTimeline = (event: SendingNetworkEvent) => {
        this.client.decryptEventIfNeeded(event);
    ...
    private eventIsACall(event: SendingNetworkEvent): boolean {
        const type = event.getType();
        /**
         * Unstable prefixes:
         *   - org.sendingnetwork.call. : MSC3086 https://github.com/sending-network/sendingnetwork-doc/pull/3086
         */
        return type.startsWith("m.call.") || type.startsWith("org.sendingnetwork.call.");
    }

    private async handleCallEvent(event: SendingNetworkEvent) {
        ...
        if (type === EventType.CallInvite) {
            ...
            if (call) {
                logger.log(
                    `WARN: Already have a SendingNetworkCall with id ${content.call_id} but got an ` +
                    `invite. Clobbering.`,
                );
            }
            ...
            call = createNewSendingNetworkCall(
                this.client,
                event.getRoomId(),
            ...
            if (!call) {
                ...
                call = createNewSendingNetworkCall(this.client, event.getRoomId());
                if (call) {
                    call.callId = content.call_id;
