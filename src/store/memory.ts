...
import { IEvent, SendingNetworkEvent } from "../models/event";
import { RoomState } from "../models/room-state";
import { RoomMember } from "../models/room-member";
import { Filter } from "../filter";
import { IAssetCache, ISavedSync, IStore } from "./index";
...
/**
 * Construct a new in-memory data store for the Sendingnetwork Client.
 * @constructor
...
export class MemoryStore implements IStore {
    ...
    public accountData: Record<string, SendingNetworkEvent> = {}; // type : content
    private readonly localStorage: Storage;
    private oobMembers: Record<string, IEvent[]> = {}; // roomId: [member events]
    private clientOptions = {};
    private assets: IAssetCache;
     ...
     */
    private onRoomMember = (event: SendingNetworkEvent, state: RoomState, member: RoomMember) => {
     ...
     * @param {Room} room The sendingnetwork room
     * @param {integer} limit The max number of old events to retrieve.
     * @return {Array<Object>} An array of objects which will be at most 'limit'
     * length and at least 0. The objects are the raw event JSON.
     */
    public scrollback(room: Room, limit: number): SendingNetworkEvent[] {
        return [];
    }
     ...
     * @param {Array<SendingNetworkEvent>} events The events to store.
     * @param {string} token The token associated with these events.
     * @param {boolean} toStart True if these are paginated results.
     */
    public storeEvents(room: Room, events: SendingNetworkEvent[], token: string, toStart: boolean) {
     ...
     * @param {Array<SendingNetworkEvent>} events The events to store.
     */
    public storeAccountDataEvents(events: SendingNetworkEvent[]): void {
     ...
     * @return {?SendingNetworkEvent} the user account_data event of given type, if any
     */
    public getAccountData(eventType: EventType | string): SendingNetworkEvent | undefined {
    ...
    public getAssets(): Promise<IAssetCache> {
        return Promise.resolve(this.assets);
    }

    public storeAssets(assets: IAssetCache): Promise<void> {
        this.assets = assets;
        return Promise.resolve();
    }
}
