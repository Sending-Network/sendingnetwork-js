...
/**
 * This is an internal module. See {@link SendingNetworkClient} for the public class.
 * @module client
 */

import { EventEmitter } from "events";
import { ISyncStateData, SyncApi } from "./sync";
import { EventStatus, IContent, IDecryptOptions, IEvent, SendingNetworkEvent } from "./models/event";
import { StubStore } from "./store/stub";
import { createNewSendingNetworkCall, SendingNetworkCall } from "./webrtc/call";
...
import { IAssetCache, IStore } from "./store";
...
export interface ExportedDeviceDecoder {
    decodeExportedDevice(data: Uint8Array): Promise<IExportedDevice | null>;
}

export interface ExportedDeviceEncoder {
    encodeExportedDevice(exportedDevice: Readonly<IExportedDevice>): Promise<Uint8Array>;
}

export interface ExportedDeviceStore extends ExportedDeviceEncoder, ExportedDeviceDecoder {
}
...
export interface ICreateClientOpts {
    ...
    /**
     * The data store used for sync data from the node. If not specified,
     ...
     * {@link module:scheduler~SendingNetworkScheduler#setProcessFunction}.
     */
    scheduler?: SendingNetworkScheduler;
     ...
     * improved timeline support ({@link module:client~SendingNetworkClient#getEventTimeline getEventTimeline}). It is
    ...
    /**
     * True to advertise support for call transfers to other parties on Sendingnetwork calls. Default false.
     */
    supportsCallTransfer?: boolean;

    /**
     * Whether to allow a fallback ICE server should be used for negotiating a
     * WebRTC connection if the node doesn't provide any servers. Defaults to false.
     */
...
export interface ISendingNetworkClientCreateOpts extends ICreateClientOpts {
    /**
...
export interface IHiSeasLoginOptions {
    /**
     * Determine whether we should use saved device ID.
     */
...
export interface IStartClientOpts {
    ...
    /**
     * The number of seconds between polls to /.well-known/api/client, undefined to disable.
...
export interface IRoomVersionsCapability {
    ...
    "org.sendingnetwork.msc3244.room_capabilities"?: Record<string, IRoomCapability>; // MSC3244
}
...
export interface IHiSeasPrivacyCapability {
    /**
     * Determine whether E2E encrypted is forced by the server.
     */
    // eslint-disable-next-line camelcase
    force_e2e_encryption: boolean;
    /**
     * The message duration on this server in milliseconds.
     */
    // eslint-disable-next-line camelcase
    messages_retention_duration: number;
}
...
interface ICapabilities {
    ...
    "im.hiseas.privacy"?: IHiSeasPrivacyCapability;
}
...
export interface IPreviewUrlResponse {
    ...
    "sendingnetwork:image:size"?: number;
}
...
interface IUploadKeysRequest {
    ...
    "org.sendingnetwork.msc2732.fallback_keys"?: Record<string, IOneTimeKey>;
}

interface IOpenIDToken {
    access_token: string;
    token_type: "Bearer" | string;
    sendingnetwork_server_name: string;
    expires_in: number;
}
...
export interface IInstance {
    ...
    fields: object;
    network_id: string;
    // XXX: this is undocumented but we rely on it: https://github.com/sending-network/sendingnetwork-doc/issues/3203
    instance_id: string;
}
...
/**
 * Represents a Sendingnetwork Client. Only directly construct this if you want to use
 * custom modules. Normally, {@link createClient} should be used
 * as it specifies 'sensible' defaults for these modules.
 */
export class SendingNetworkClient extends EventEmitter {
    ...
    public scheduler: SendingNetworkScheduler;
    ...
    public http: SendingNetworkHttpApi; // XXX: Intended private, used in code.
    ...
    // TODO: This should expire: https://github.com/sending-network/sendingnetwork-js-sdk/issues/1020
    ...
    protected loginOptions: IHiSeasLoginOptions;
    protected wallet: any;

    constructor(opts: ISendingNetworkClientCreateOpts) {
        super();
        ...
        this.http = new SendingNetworkHttpApi(this, {
        ...
        // try constructing a SendingNetworkCall to see if we are running in an environment
        // which has WebRTC. If we are, listen for and handle m.call.* events.
        const call = createNewSendingNetworkCall(this, undefined, undefined);
        if (call) {
     ...
     * events, add a listener for {@link module:client~SendingNetworkClient#event:"event"}
     * via {@link module:client~SendingNetworkClient#on}. Alternatively, listen for specific
     * state change events.
    ...
    public async startClient(opts: IStartClientOpts) {
        ...
        if (window.localStorage["m.login_options"]) {
            this.loginOptions = JSON.parse(window.localStorage["m.login_options"]);
        }

        if (window.localStorage["m.wallet"]) {
            this.wallet = JSON.parse(window.localStorage["m.wallet"]);
        }
    }
     ...
     */
    public stopClient() {
        logger.log('stopping SendingNetworkClient');
    ...
    public setLoginOptions(opts: IHiSeasLoginOptions) {
        this.loginOptions = opts;
        window.localStorage["m.login_options"] = JSON.stringify(opts);
    }
    ...
    public setWallet(wallet: any,
        props: string[] = ["walletChain", "walletNetwork", "walletProvider", "walletEndpoint"]) {
        if (wallet) {
            window.localStorage["m.wallet"] = JSON.stringify(wallet, props);
        }
        this.wallet = wallet;
    }
     ...
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    public async rehydrateDevice(): Promise<string> {
        ...
        try {
            ...
            const rehydrateResult = await this.http.authedRequest(
                ...
                {
                    prefix: "/_api/client/unstable/org.sendingnetwork.msc2697.v2",
                },
            );
    ...
    /**
     * Set whether to advertise transfer support to other parties on Sendingnetwork calls.
     ...
     * @return {SendingNetworkCall} the call or null if the browser doesn't support calling.
     */
    public createCall(roomId: string): SendingNetworkCall {
        return createNewSendingNetworkCall(this, roomId);
    }

    /**
     * Get the current sync state.
     * @return {?SyncState} the sync state, which may be null.
     * @see module:client~SendingNetworkClient#event:"sync"
     */
     ...
     * @return {?module:scheduler~SendingNetworkScheduler} The scheduler or null
     */
    public getScheduler(): SendingNetworkScheduler {
    ...
    /**
     * Gets the capabilities of the node. Always returns an object of
     * capability keys and their options, which may be empty.
     * @param {boolean} fresh True to ignore any cached values.
     * @return {Promise} Resolves to the capabilities of the node
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
     ...
     *
     * You should call this method after creating the sendingNetworkclient, but *before*
    ...
    public async initCrypto(): Promise<void> {
        ...
        if (this.crypto) {
            logger.warn("Attempt to re-initialise e2e encryption on SendingNetworkClient");
            return;
        }
        ...
        if (userId === null) {
            throw new Error(
                `Cannot enable encryption on SendingNetworkClient with unknown userId: ` +
                `ensure userId is passed in createClient().`,
            );
        }
        if (this.deviceId === null) {
            throw new Error(
                `Cannot enable encryption on SendingNetworkClient with unknown deviceId: ` +
    ...
    /**
     * Upload the device keys to the node.
     ...
     * @returns {Promise}
     *
     * @fires module:client~event:SendingNetworkClient"deviceVerificationChanged"
     */
     ...
     * The cross-signing API is currently UNSTABLE and may change without notice.
     *
     * @function module:client~SendingNetworkClient#checkDeviceTrust
     ...
     *
     * @param {module:models/event.SendingNetworkEvent} event event to be checked
     * @returns {IEncryptedEventInfo} The event information.
     */
    public getEventEncryptionInfo(event: SendingNetworkEvent): IEncryptedEventInfo {
        if (!this.crypto) {
            throw new Error("End-to-end encryption disabled");
        }
     ...
     *
     * @param {SendingNetworkEvent} event event to be checked
     *
     * @return {Promise<module:crypto/deviceinfo?>}
     */
    public async getEventSenderDeviceInfo(event: SendingNetworkEvent): Promise<DeviceInfo> {
        if (!this.crypto) {
            return null;
        }
     ...
     * {@link module:client~SendingNetworkClient#setDeviceVerified|setDeviceVerified}.
     */
    public async isEventSenderVerified(event: SendingNetworkEvent): Promise<boolean> {
     ...
     * request.
     * @param  {SendingNetworkEvent} event event of which to cancel and resend the room
     *                            key request.
     * @return {Promise} A promise that will resolve when the key request is queued
     */
    public cancelAndResendEventRoomKeyRequest(event: SendingNetworkEvent): Promise<void> {
    ...
    /**
     * Back up session keys to the node.
    ...
    private async restoreKeyBackup(
        ...
        try {
            ...
            if (!await algorithm.keyMatches(privKey)) {
                return Promise.reject(new SendingNetworkError({ errcode: SendingNetworkClient.RESTORE_BACKUP_ERROR_BAD_KEY }));
            }
    ...
    public getVisibleRooms(): Room[] {
        ...
        return allRooms.filter((r) => {
            ...
            return true;
        }).filter(r=>r.getMyMembership() !== 'leave');
    }
     ...
     */
    public getAccountData(eventType: string): SendingNetworkEvent {
        return this.store.getAccountData(eventType);
    }

    /**
     * Get account data event of given type for the current user. This variant
     * gets account data directly from the node if the local store is not
     ...
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    public setRemarkedUsers(userIds: string[], callback?: Callback): Promise<{}> {
        const content = { ignored_users: {} };
        userIds.map((u) => content.ignored_users[u] = {});
        return this.setAccountData("m.remarked_user_list", content, callback);
    }
    ...
    public async joinRoom(roomIdOrAlias: string, opts?: IJoinRoomOpts, callback?: Callback): Promise<Room> {
        ...
        try {
            ...
            if (!res.room_id && res.qualified === false) {
                return res;
            }
    ...
    /**
     * Resend an event.
     * @param {SendingNetworkEvent} event The event to resend.
     * @param {Room} room Optional. The room the event is in. Will update the
     * timeline entry if provided.
     * @return {Promise} Resolves: to an ISendEventResponse object
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    public resendEvent(event: SendingNetworkEvent, room: Room): Promise<ISendEventResponse> {
     ...
     *
     * @param {SendingNetworkEvent} event   Event to cancel
     * @throws Error if the event is not in QUEUED or NOT_SENT state
     */
    public cancelPendingEvent(event: SendingNetworkEvent) {
     ...
     * @param {SendingNetworkEvent} event
     * @param {module:client.callback} callback Optional.
     * @return {Promise} Resolves: to an ISendEventResponse object
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    ...
    private sendCompleteEvent(
        ...
        // we always construct a SendingNetworkEvent when sending because the store and
        // scheduler use them. We'll extract the params back out if it turns out
        // the client has no scheduler or store.
        const localEvent = new SendingNetworkEvent(Object.assign(eventObject, {
     ...
     * @private
     */
    private encryptAndSendEvent(room: Room, event: SendingNetworkEvent, callback?: Callback): Promise<ISendEventResponse> {
    ...
    private encryptEventIfNeeded(event: SendingNetworkEvent, room?: Room): Promise<void> | null {
        ...
        if (!this.crypto && this.usingExternalCrypto) {
            ...
            // crypto. This is useful for users of sending-network/pantalaimon
            return null;
        }
        ...
        if (event.getType() === EventType.Reaction) {
            ...
            // See https://github.com/sending-network/sendingnetwork-doc/pull/1849#pullrequestreview-248763642
            return null;
        }
     ...
     * @param {SendingNetworkClient} client the client
    ...
    private updatePendingEventStatus(room: Room | null, event: SendingNetworkEvent, newStatus: EventStatus) {
        if (room) {
    ...
    private sendEventHttpRequest(event: SendingNetworkEvent): Promise<ISendEventResponse> {
     ...
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    public sendReceipt(event: SendingNetworkEvent, receiptType: string, body: any, callback?: Callback): Promise<{}> {
     ...
     * other users and nodes. Default false (send to everyone). <b>This
     * property is unstable and may change in the future.</b>
     * @param {module:client.callback} callback Optional.
     * @return {Promise} Resolves: to an empty object
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    public async sendReadReceipt(event: SendingNetworkEvent, opts?: { hidden?: boolean }, callback?: Callback): Promise<{}> {
        ...
        const addlContent = {
            "org.sendingnetwork.msc2285.hidden": Boolean(opts.hidden),
        };
     ...
     * @param {SendingNetworkEvent} rrEvent the event tracked by the read receipt. This is here for
     * convenience because the RR and the RM are commonly updated at the same time as each
     * other. The local echo of this receipt will be done if set. Optional.
     * @param {object} opts Options for the read markers
     * @param {object} opts.hidden True to hide the receipt from other users and nodes.
    ...
    public async inviteByThreePid(roomId: string, medium: string, address: string, callback?: Callback): Promise<{}> {
        ...
        if (!identityServerUrl) {
            return Promise.reject(new SendingNetworkError({
                error: "No supplied identity server URL",
                errcode: "ORG.SENDINGNETWORK.JSSDK_MISSING_PARAM",
            }));
        }
        const params = {
    ...
    public leaveRoomChain(
        roomId: string,
        includeFuture = true,
    ): Promise<{ [roomId: string]: Error | SendingNetworkError | null }> {
     ...
     * @param {SendingNetworkEvent} event The event to get push actions for.
     * @return {module:pushprocessor~PushAction} A dict of actions to perform.
     */
    public getPushActionsForEvent(event: SendingNetworkEvent): IActionsObject {
    ...
    public setProfileInfo(info: "signature", data: { signature: string }, callback?: Callback): Promise<{}>;
    public setProfileInfo(info: "avatar_url" | "displayname" | "signature", data: object, callback?: Callback): Promise<{}> {
    ...
    public async setSignature(aboutMe: string, callback?: Callback): Promise<{}> {
        const prom = await this.setProfileInfo("signature", { signature: aboutMe }, callback);
        const user = this.getUser(this.getUserId());
        if (user) {
            user.signature = aboutMe;
            user.emit("User.signature", user.events.presence, user);
        }
        return prom;
    }
    ...
    public scrollback(room: Room, limit: number, callback?: Callback): Promise<Room> {
        ...
        const prom = new Promise<Room>((resolve, reject) => {
            ...
            sleep(timeToWaitMs).then(() => {
                ...
                const sendingNetworkEvents = res.chunk.map(this.getEventMapper());
                if (res.state) {
                    const stateEvents = res.state.map(this.getEventMapper());
                    room.currentState.setUnknownStateEvents(stateEvents);
                }
                room.addEventsToTimeline(sendingNetworkEvents, true, room.getLiveTimeline());
                room.oldState.paginationToken = res.end;
                if (res.chunk.length === 0) {
                    room.oldState.paginationToken = null;
                }
                this.store.storeEvents(room, sendingNetworkEvents, res.end, true);
    ...
    public getEventTimeline(timelineSet: EventTimelineSet, eventId: string): Promise<EventTimeline> {
        ...
        if (!this.timelineSupport) {
            throw new Error("timeline support is disabled. Set the 'timelineSupport'" +
                " parameter to true when creating SendingNetworkClient to enable" +
                " it.");
        }
        ...
        const promise = this.http.authedRequest(undefined, "GET", path, params).then((res) => {
            if (!res.event) {
                throw new Error("'event' not in '/context' result - node too old?");
            }
            ...
            const sendingNetworkEvents = events.map(this.getEventMapper());

            let timeline = timelineSet.getTimelineForEvent(sendingNetworkEvents[0].getId());
            if (!timeline) {
            ...
            timelineSet.addEventsToTimeline(sendingNetworkEvents, true, timeline, res.start);
    ...
    public createMessagesRequest(
        ...
        if (timelineFilter) {
            ...
            // /sync's one - see https://sending.network/jira/browse/SPEC-451
            filter = filter || {};
        ...
        if (isNotifTimeline) {
            ...
            promise = this.http.authedRequest(
                ...
                for (let i = 0; i < res.notifications.length; i++) {
                    ...
                    sendingNetworkEvents[i] = event;
                }

                eventTimeline.getTimelineSet()
                    .addEventsToTimeline(sendingNetworkEvents, backwards, eventTimeline, token);
     ...
     * This API requests a token from the node.
     ...
     * or return M_THREEPID_IN_USE (which one is up to the node).
     *
     * @param {string} email As requestEmailToken
     * @param {string} clientSecret As requestEmailToken
     * @param {number} sendAttempt As requestEmailToken
     ...
     * to the address informing them of this (which one is up to the node).
     *
     ...
     * The operation also updates SendingNetworkClient.pushRules at the end.
     * @param {string} scope "global" or device-specific.
     * @param {string} roomId the id of the room.
    ...
    /**
     * Gets a bearer token from the node that the user can
     * present to a third party in order to prove their ownership
     * of the Sendingnetwork account they are logged into.
    ...
    /**
     * Get the TURN servers for this node.
     ...
     * WebRTC connection if the node doesn't provide any servers. Defaults to
     * false.
     *
     * @param {boolean} allow
     */
    ...
    /**
     * Determines if the current user is an administrator of the Synapse node.
     * Returns false if untrue or the node does not appear to be a Synapse
     * node. <strong>This function is implementation specific and may change
     * as a result.</strong>
     ...
     * passed to the node. Some nodes may trigger errors if they are not
     ...
     * @return {Object} an object with `events` as `SendingNetworkEvent[]` and optionally `nextBatch` if more relations are available.
     */
    public async relations(
        ...
        eventType: string,
        opts: { from: string },
    ): Promise<{ originalEvent: SendingNetworkEvent, events: SendingNetworkEvent[], nextBatch?: string }> {
     ...
     * @param {SendingNetworkEvent} event The event to decrypt
     ...
     */
    public decryptEventIfNeeded(event: SendingNetworkEvent, options?: IDecryptOptions): Promise<void> {
    ...
    private termsUrlForService(serviceType: SERVICE_TYPES, baseUrl: string) {
        switch (serviceType) {
            ...
            case SERVICE_TYPES.IM:
                return baseUrl + '/_api/integrations/v1/terms';
            default:
    ...
    /**
     * Get the Node URL of this client
     * @return {string} Node URL of this client
     */
     ...
     * ```javascript
     * const tmpClient = await sdk.createClient(SENDINGNETWORK_INSTANCE);
     * const { user_id, device_id, access_token } = tmpClient.registerGuest();
     * const client = createClient({
     *   baseUrl: SENDINGNETWORK_INSTANCE,
    ...
    /**
     * @param {string} token Login token previously received from node
     * @param {module:client.callback} callback Optional.
     * @return {Promise} Resolves: TODO
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
     ...
     * method is called. The state of the SendingNetworkClient object is not affected:
     * it is up to the caller to either reset or destroy the SendingNetworkClient after
     ...
     * @param {string} authSessionId the auth session ID provided by the node
     *
    ...
    public setRoomReadMarkersHttpRequest(
        ...
        const content = {
            ...
            "m.read": rrEventId,
            "org.sendingnetwork.msc2285.hidden": Boolean(opts ? opts.hidden : false),
        };
    ...
    public unstableGetLocalAliases(roomId: string, callback?: Callback): Promise<{ aliases: string[] }> {
        const path = utils.encodeUri("/rooms/$roomId/aliases",
            { $roomId: roomId });
        const prefix = PREFIX_UNSTABLE + "/org.sendingnetwork.msc2432";
    ...
    /**
     * Upload a file to the media repository on the node.
     *
     ...
     *    opts.onlyContentUri.  Rejects with an error (usually a SendingNetworkError).
     */
    ...
    public getProfileInfo(
        ...
        avatar_url?: string; displayname?: string; threepids?: IThreepid[]; wallet_address?: string; signature?: string;}> {
    ...
    /**
     * Add a 3PID to your node account and optionally bind it to an identity
    ...
    /**
     * Add a 3PID to your node account. This API does not use an identity
     * server, as the node is expected to handle 3PID ownership validation.
     *
     * You can check whether a node supports this API via
     * `doesServerSupportSeparateAddAndBind`.
     *
     * @param {Object} data A object with 3PID validation data from having called
     * `account/3pid/<medium>/requestToken` on the node.
     * @return {Promise} Resolves: on success
     * @return {module:http-api.SendingNetworkError} Rejects: with an error response.
     */
    ...
    /**
     * Bind a 3PID for discovery onto an identity server via the node. The
     * identity server handles 3PID ownership validation and the node records
    ...
    /**
     * Unbind a 3PID for discovery on an identity server via the node. The
     * node removes its record of the binding to keep an updated record of
     ...
     *     an error response ({@link module:http-api.SendingNetworkError}).
     */
     ...
     * Node, which can be retrieved via
     * {@link module:client~SendingNetworkClient#getOpenIdToken}.
     *
     ...
     * newer 3PID flow where the node validates 3PID ownership (as part of
     * `requestAdd3pidMsisdnToken`). The node response may include a
    ...
    /**
     * Looks up the public Sendingnetwork ID mapping for a given 3rd party
    ...
    /**
     * Looks up the public Sendingnetwork ID mappings for multiple 3PIDs.
     *
    ...
    public getSpaceSummary(
        ...
        return this.http.authedRequest(undefined, "POST", path, null, {
            ...
            prefix: "/_api/client/unstable/org.sendingnetwork.msc2946",
        });
    }
     ...
     * Proposed at https://github.com/sending-network/sendingnetwork-doc/pull/3266
    ...
    public async getRoomSummary(roomIdOrAlias: string, via?: string[]): Promise<IRoomSummary> {
        ...
        return this.http.authedRequest(undefined, "GET", path, { via }, null, {
            ...
            prefix: "/_api/client/unstable/im.nheko.summary",
        });
    }

    /**
     * Fetch the transfer status by user and room and eventId
     * @param userId string
     * @param roomId string
     * @param eventId string
     * @returns Promise<any>
     */
    public getTransferStatus(roomId: string, eventId: string): Promise<any> {
        const userId = this.getUserId();
        const path = utils.encodeUri("/room/transaction/$userId", { $userId: userId });
        return this.http.authedRequest(undefined, "POST", path,
            null, { room_id: roomId, event_id: eventId });
    }
     ...
     * @returns Promise<any>
     */
    public updateTransferStatus(roomId: string, eventId: string, data: Transfer) {
        const userId = this.getUserId();
        const uri = (data.state === TransferStatus.Success)
            ? "/room/transaction_success/$userId"
            : "/room/transaction_state/$userId";
        const path = utils.encodeUri(uri, { $userId: userId });
        return this.http.authedRequest(undefined, "PUT", path,
            null, { room_id: roomId, event_id: eventId, ...data });
    }

    /**
     * create the transfer status
     * @param roomId string
     * @param eventId string
     * @param data Transfer
     * @returns Promise<any>
     */
    public createTransferStatus(roomId: string, eventId: string, data: Transfer) {
        const { type, ...other } = data;
        const userId = this.getUserId();
        const uri = '/room/transaction/$userId';
        const path = utils.encodeUri(uri, { $userId: userId });
        return this.http.authedRequest(undefined, "PUT", path,
            null, { room_id: roomId, event_id: eventId, transaction_type: type, ...other });
    }
     ...
     * @returns Promise<any>
     */
    public getWalletByUserId(users: string[]): Promise<any[]> {
        const path = '/account/3pid/did';
        return this.http.authedRequest(undefined, "POST", path,
            null, { users });
    }

    public async findDMRoomByUserId(userId: string): Promise<any> {
        const uri = '/find_dm_room/$userId';
        const path = utils.encodeUri(uri, { $userId: userId });
        return this.http.authedRequest(undefined, "GET", path,
            null, null);
    }

    public getQRCode(deviceId?: string): Promise<any> {
        return this.http.request(undefined, "GET", `/login/showQRCode`, { deviceId });
    }

    public checkQRCode(qrCode: string): Promise<any> {
        if (qrCode) {
            return this.http.request(
                undefined,
                'GET',
                `/login/checkQRCode/${qrCode}`,
            );
        }
    ...
    public getFavouriteList(): Promise<any> {
        const userId = this.getUserId();
        return this.http.authedRequest(
            undefined,
            'GET',
            `/contacts_list/${userId}`,
            null,
            null,
        );
    }

    public addFavourite(targetId: string, isDm: boolean): Promise<any> {
        const userId = this.getUserId();
        const data = { contact_id: targetId, is_room: isDm ? 0 : 1 };
        return this.http.authedRequest(
            undefined,
            'PUT',
            `/contact/${userId}`,
    ...
    public deleteFavourite(targetId: string, isDm: boolean): Promise<any> {
        const userId = this.getUserId();
        const data = { contact_id: targetId, is_room: isDm ? 0 : 1 };
        return this.http.authedRequest(
            undefined,
    ...
    public getFavourite(targetId: string, isRoom: boolean): Promise<any> {
        const userId = this.getUserId();
        const data = { contact_id: targetId, is_room: isRoom ? 1: 0 };
        return this.http.authedRequest(
            undefined,
            'POST',
            `/contact/${userId}`,
    ...
    public getAuthAppInfo(params: QueryDict): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
            `/oauth/app/info?client_id=${
                params.client_id
            }&redirect_uri=${encodeURIComponent(params.redirect_uri as string)}`,
        );
    }

    public authorize(params: QueryDict): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
    ...
    private _getAssets(address: string, cursor: string, limit: number): Promise<{ next: string, assets: IAsset[] }> {
        return this.http.requestOtherUrl(
            undefined,
            "GET",
    ...
    // Only cache part of the asset, if the required data changes, update this version
    private assetsVersion = 1;

    public async getAssets(address: string, refresh = false) {
        const limit = 50;
        let cursor = '';
        let cache: IAssetCache;

        if (!refresh) {
            cache = await this.store.getAssets();
        }
        const result: IAsset[] = [];
        const loadMore = async () => {
            const { next, assets } = await this._getAssets(address, cursor, limit);
            cursor = next;
            result.push(...assets.map(
                asset => ({
                    id: asset.id,
                    ...
                    ...asset.collection ? {
                        collection: {
                            ...
                            image_url: asset.collection.image_url,
                        },
                    } : {},
                    ...asset.creator ? {
                        creator: {
        ...
        const { next, assets } = await this._getAssets(address, cursor, limit);
        if (cache && assets[0]?.id === cache.data[0]?.id && cache.version === this.assetsVersion) {
            return cache.data;
        }
        cursor = next;
        await loadMore();
        await this.store.storeAssets({
    ...
    public getPartners(): Promise<{ name: string, url: string, avatar?: string }[]> {
        return this.http.authedRequest(
            undefined,
            "GET",
            "/preview_url_app",
    ...
    public setAccountGuide(step: number): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            "/account/guide",
    ...
    // query Social Graph
    public querySocialGraph(): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
            "/social_graph/query",
    ...
    public backupSocialGraph(data: {
        ...
        return this.http.authedRequest(
            undefined,
            "POST",
            "/social_graph/backup_transaction",
            null,
            { ...data },
            {
                prefix: "/_api/client/unstable",
    ...
    public restoreSocialGraph(data: { signature?: string }): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            "/social_graph/restore_account",
            null,
            { ...data },
            {
                prefix: "/_api/client/unstable",
    ...
    // did model
    public getDIDList(address): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
            `/address/${address}`,
    ...
    // create did
    public createDID(address: string): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            `/did/create`,
    ...
    // save did
    public saveDID(did: string, data: any): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            `/did/${did}`,
    ...
    public preDIDLogin(did: string): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            `/did/${did}/pre_login`,
    ...
    public queryDIDDetail(did): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
            `/did/${did}/detail`,
    ...
    public updateDID(did: string, data: any): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            `/did/${did}/update`,
    ...
    public resolveDID(did): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
            `/did/${did}`,
    ...
    // ntf
    public createNftRoomToken(roomID: any): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            `/${roomID}/roomToken`,
            ...
            null,
            {
                prefix: "/_api/client/v3/rooms",
            },
        );
    }

    public delNftRoomToken(roomID: any): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "DELETE",
            `/${roomID}/roomToken`,
    ...
    public createNftMemberToken(roomID: any): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "POST",
            `/${roomID}/memberToken`,
    ...
    public delNftMemberToken(roomID: any): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "DELETE",
            `/${roomID}/memberToken`,
    ...
    // dapp
    public queryMessageType(): Promise<any> {
        return this.http.authedRequest(
            undefined,
            "GET",
            `/query_message_type`,
            ...
            null,
            {
                prefix: "/_api/media/v3",
            },
        );
    }

    public getUrlPreviewNew(url: string, ts: number, callback?: Callback): Promise<IPreviewUrlResponse> {
        // bucket the timestamp to the nearest minute to prevent excessive spam to the server
        // Surely 60-second accuracy is enough for anyone.
        ts = Math.floor(ts / 60000) * 60000;
        ...
        const resp = this.http.authedRequest(
            callback, "GET", "/preview_url_message_type", {
                url: url,
                ts: ts,
            }, undefined, {
                prefix: '/_api/media/v3',
 ...
 *
 * @event module:client~SendingNetworkClient#"event"
 * @param {SendingNetworkEvent} event The sendingnetwork event which caused this event to fire.
 * @example
 * sendingNetworkClient.on("event", function(event){
 ...
 * @event module:client~SendingNetworkClient#"toDeviceEvent"
 * @param {SendingNetworkEvent} event The sendingnetwork event which caused this event to fire.
 * @example
 * sendingNetworkClient.on("toDeviceEvent", function(event){
 *   var sender = event.getSender();
 * });
 */
 ...
 * </ul>
 *
 * @event module:client~SendingNetworkClient#"sync"
 *
 ...
 *
 * @param {SendingNetworkError} data.error The sendingnetwork error if <code>state=ERROR</code>.
 *
 ...
 *
 * @example
 * sendingNetworkClient.on("sync", function(state, prevState, data) {
 *   switch (state) {
 *     case "ERROR":
 ...
 *       var rooms = sendingNetworkClient.getRooms();
 *       break;
 *   }
 * });
 */
 ...
 * @event module:client~SendingNetworkClient#"Group"
 * @param {Group} group The newly created, fully populated group.
 * @deprecated groups/communities never made it to the spec and support for them is being discontinued.
 * @example
 * sendingNetworkClient.on("Group", function(group){
 ...
 * may change.</strong>
 * @event module:client~SendingNetworkClient#"Room"
 * @param {Room} room The newly created, fully populated room.
 * @example
 * sendingNetworkClient.on("Room", function(room){
 ...
 * @event module:client~SendingNetworkClient#"deleteRoom"
 * @param {string} roomId The deleted room ID.
 * @example
 * sendingNetworkClient.on("deleteRoom", function(roomId){
 ...
 * @event module:client~SendingNetworkClient#"Call.incoming"
 * @param {module:webrtc/call~SendingNetworkCall} call The incoming call.
 * @example
 * sendingNetworkClient.on("Call.incoming", function(call){
 ...
 * @event module:client~SendingNetworkClient#"Session.logged_out"
 * @example
 * sendingNetworkClient.on("Session.logged_out", function(errorObj){
 ...
 * to a HTTP request.
 * @event module:client~SendingNetworkClient#"no_consent"
 * @example
 * sendingNetworkClient.on("no_consent", function(message, contentUri) {
 ...
 * {@link module:client~SendingNetworkClient#setDeviceVerified|SendingNetworkClient.setDeviceVerified} or
 * {@link module:client~SendingNetworkClient#setDeviceBlocked|SendingNetworkClient.setDeviceBlocked}.
 *
 * @event module:client~SendingNetworkClient#"deviceVerificationChanged"
 ...
 * The cross-signing API is currently UNSTABLE and may change without notice.
 *
 * @event module:client~SendingNetworkClient#"userTrustStatusChanged"
 ...
 * The cross-signing API is currently UNSTABLE and may change without notice.
 *
 * @event module:client~SendingNetworkClient#"crossSigning.keysChanged"
 */

/**
 * Fires whenever new user-scoped account_data is added.
 * @event module:client~SendingNetworkClient#"accountData"
 * @param {SendingNetworkEvent} event The event describing the account_data just added
 * @param {SendingNetworkEvent} event The previous account data, if known.
 * @example
 * sendingNetworkClient.on("accountData", function(event, oldEvent){
 ...
 * @event module:client~SendingNetworkClient#"crypto.devicesUpdated"
 ...
 * @event module:client~SendingNetworkClient#"crypto.willUpdateDevices"
 ...
 * @event module:client~SendingNetworkClient#"crypto.keyBackupStatus"
 * @param {boolean} enabled true if key backup has been enabled, otherwise false
 * @example
 * sendingNetworkClient.on("crypto.keyBackupStatus", function(enabled){
 *   if (enabled) {
 *     [...]
 *   }
 * });
 ...
 *
 * @event module:client~SendingNetworkClient#"crypto.suggestKeyRestore"
 */

/**
 * Fires when a key verification is requested.
 * @event module:client~SendingNetworkClient#"crypto.verification.request"
 * @param {object} data
 * @param {SendingNetworkEvent} data.event the original verification request message
 ...
 * @event module:client~SendingNetworkClient#"crypto.verification.request.unknown"
 ...
 * The Secure Secret Storage API is currently UNSTABLE and may change without notice.
 *
 * @event module:client~SendingNetworkClient#"crypto.secrets.requestCancelled"
 * @param {object} data
 ...
 *
 * @event module:client~SendingNetworkClient#"WellKnown.client"
