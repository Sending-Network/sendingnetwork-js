...
interface IMessage {
    ...
    content: {
        ...
        "org.sendingnetwork.msc3061.shared_history": boolean;
    };
}
...
class MegolmRatchetEncryption extends EncryptionAlgorithm {
    ...
    public async reshareKeyWithDevice(
        ...
        const payload = {
            ...
            content: {
                ...
                "org.sendingnetwork.msc3061.shared_history": key.shared_history || false,
            },
        };
    ...
    public prepareToEncrypt(room: Room): void {
        if (this.encryptionPreparation) {
            ...
            // (https://github.com/sending-network/sendingnetwork-js-sdk/issues/1255)
...
class MegolmRatchetDecryption extends DecryptionAlgorithm {
    ...
    public async decryptEvent(event: SendingNetworkEvent): Promise<IEventDecryptionResult> {
        ...
        try {
            res = await this.olmDevice.decryptGroupMessage(
                event.getRoomId(), content.algorithm, content.sender_key, content.session_id, content.ciphertext,
     ...
     */
    private addEventToPendingList(event: SendingNetworkEvent): void {
        const content = event.getWireContent();
     ...
     * @param {module:models/event.SendingNetworkEvent} event
     */
    private removeEventFromPendingList(event: SendingNetworkEvent): void {
        const content = event.getWireContent();
        const senderKey = content.sender_key;
        const sessionId = content.session_id;
        const senderPendingEvents = this.pendingEvents[senderKey];
     ...
     */
    public onRoomKeyEvent(event: SendingNetworkEvent): Promise<void> {
        ...
        if (content["org.sendingnetwork.msc3061.shared_history"]) {
     ...
     * @param {module:models/event.SendingNetworkEvent} event key event
     */
    public async onRoomKeyWithheldEvent(event: SendingNetworkEvent): Promise<void> {
        const content = event.getContent();
        const senderKey = content.sender_key;
    ...
    public importRoomKey(session: IMegolmSessionData, opts: any = {}): Promise<void> {
        ...
        if (session["org.sendingnetwork.msc3061.shared_history"]) {
            extraSessionData.sharedHistory = true;
        }
        return this.olmDevice.addInboundGroupSession(
            session.room_id,
