...
import { SendingNetworkEvent, EventStatus } from "../models/event";
import { SendingNetworkClient, IKeysUploadResponse, SessionStore, ISignedKey } from "../client";
...
export class Crypto extends EventEmitter {
     ...
     * Cryptography bits
     *
     * This module is internal to the js-sdk; the public API is via SendingNetworkClient.
     *
     * @constructor
     * @alias module:crypto
     *
     * @internal
     *
     * @param {SendingNetworkClient} baseApis base sendingnetwork api interface
     *
     ...
     *
     * @param {Object} clientStore the SendingNetworkClient data store.
     *
     ...
     *    Each element can either be a string from SendingNetworkClient.verificationMethods
     *    or a class that implements a verification method.
     */
    constructor(
        public readonly baseApis: SendingNetworkClient,
    ...
    /**
     * Tell the crypto module to register for SendingNetworkClient events which it needs to
     * listen for
     *
    ...
    public registerEventHandlers(eventEmitter: EventEmitter): void {
        eventEmitter.on("RoomMember.membership", (event: SendingNetworkEvent, member: RoomMember, oldMembership?: string) => {
            try {
    ...
    private async uploadOneTimeKeys() {
        ...
        const res = await this.baseApis.uploadKeysRequest({
            ...
            "org.sendingnetwork.msc2732.fallback_keys": fallbackJson,
        });
     ...
     */
    public getEventSenderDeviceInfo(event: SendingNetworkEvent): DeviceInfo | null {
    ...
    public async setRoomEncryption(
        ...
        if (!this.lazyLoadMembers) {
            ...
            // TODO: this flag is only not used from SendingNetworkClient::setRoomEncryption
     ...
     *
     * @param {module:models/event.SendingNetworkEvent} event  event to be sent
     *
    ...
    public async encryptEvent(event: SendingNetworkEvent, room: Room): Promise<void> {
        if (!room) {
        ...
        if (!alg) {
            ...
            throw new Error(
                ...
                "no longer. Perhaps the node is hiding the " +
                "configuration event.",
            );
        }
     ...
     */
    public async decryptEvent(event: SendingNetworkEvent): Promise<IEventDecryptionResult> {
        if (event.isRedacted()) {
            const redactionEvent = new SendingNetworkEvent(event.getUnsigned().redacted_because);
     ...
     *
     * @param {module:models/event.SendingNetworkEvent} event encryption event
     */
    public async onCryptoEvent(event: SendingNetworkEvent): Promise<void> {
        const roomId = event.getRoomId();
     ...
     *
     * @param {Object} syncData  the data from the 'SendingNetworkClient.sync' event
     */
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event key event
     */
    private onRoomKeyEvent(event: SendingNetworkEvent): void {
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event key withheld event
     */
    private onRoomKeyWithheldEvent(event: SendingNetworkEvent): void {
        const content = event.getContent();
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event verification start event
     */
    private onKeyVerificationMessage(event: SendingNetworkEvent): void {
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event the timeline event
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event undecryptable event
     */
    private async onToDeviceBadEncrypted(event: SendingNetworkEvent): Promise<void> {
        const content = event.getWireContent();
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event  event causing the change
     * @param {module:models/room-member} member  user whose membership changed
     * @param {string=} oldMembership  previous membership
     */
    private onRoomMembership(event: SendingNetworkEvent, member: RoomMember, oldMembership?: string): void {
     ...
     *
     * @private
     * @param {module:models/event.SendingNetworkEvent} event key request event
     */
    private onRoomKeyRequestEvent(event: SendingNetworkEvent): void {
        const content = event.getContent();
...
export class IncomingRoomKeyRequest {
    ...
    constructor(event: SendingNetworkEvent) {
        const content = event.getContent();
 ...
 *    event.  See {@link module:models/event.SendingNetworkEvent#getSenderKey}.
 *
 * @property {?string} claimedEd25519Key ed25519 key claimed by the sender of
 *    this event. See
 *    {@link module:models/event.SendingNetworkEvent#getClaimedEd25519Key}.
 *
 ...
 *     {@link module:models/event.SendingNetworkEvent#getForwardingCurve25519KeyChain}.
 */

/**
 * Fires when we receive a room key request
 *
 * @event module:client~SendingNetworkClient#"crypto.roomKeyRequest"
 ...
 *
 * @event module:client~SendingNetworkClient#"crypto.roomKeyRequestCancellation"
 ...
 *
 * @event module:client~SendingNetworkClient#"crypto.warning"
