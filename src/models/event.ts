...
/**
 * This is an internal module. See {@link SendingNetworkEvent} and {@link RoomEvent} for
 * the public classes.
 * @module models/event
 */
...
export class SendingNetworkEvent extends EventEmitter {
    private pushActions: IActionsObject = null;
    private _replacingEvent: SendingNetworkEvent = null;
    private _localRedactionEvent: SendingNetworkEvent = null;
    ...
    /**
     * Construct a Sendingnetwork Event object
     * @constructor
     *
     ...
     * from changes to event JSON between Sendingnetwork versions.
     *
    ...
    constructor(public event: Partial<IEvent> = {}) {
        ...
        // intern the values of sendingnetwork events to force share strings and reduce the
     ...
     * @return {string} The user ID, e.g. <code>@alice:sending.network</code>
     */
     ...
     * @return {string} The room ID, e.g. <code>!cURbafjkfsMDVwdRDQ:sending.network
     * </code>
     */
    ...
    public get tagEventId(): string {
        const relatesTo = this.getWireContent()?.["m.relates_to"];
        if (relatesTo?.rel_type === RelationType.Tag) {
            return relatesTo.event_id;
        }
    }
     ...
     *   See {@link module:models/event.SendingNetworkEvent#getSenderKey}.
     *
     * @param {string} claimedEd25519Key claimed ed25519 key to record for the
     *   sender if this event.
     *   See {@link module:models/event.SendingNetworkEvent#getClaimedEd25519Key}
     */
     ...
     * @internal
     *
     * @fires module:models/event.SendingNetworkEvent#"Event.decrypted"
     *
     ...
     * instead use SendingNetworkClient.getEventSenderDeviceInfo.
     *
     * @return {string}
     */
    ...
    public markLocallyRedacted(redactionEvent: SendingNetworkEvent): void {
     ...
     *
     * @param {module:models/event.SendingNetworkEvent} redactionEvent
     *     event causing the redaction
     */
    public makeRedacted(redactionEvent: SendingNetworkEvent): void {
        // quick sanity-check
     ...
     *
     * @fires module:models/event.SendingNetworkEvent#"Event.replaced"
     *
     * @param {SendingNetworkEvent?} newEvent the event with the replacing content, if any.
     */
    public makeReplaced(newEvent?: SendingNetworkEvent): void {
    ...
    public makeTag(newTag: MsgTag): void {
        if (this.event.content?.msgtag !== newTag) {
            this.event.content = this.event.content || {};
            this.event.content.msgtag = newTag;
            this.emit("Event.tagChanged", this);
        }
    }
     ...
     */
    public replacingEvent(): SendingNetworkEvent | undefined {
     ...
     * @return {SendingNetworkEvent} the event
     */
    public localRedactionEvent(): SendingNetworkEvent | undefined {
     ...
     * properties of this SendingNetworkEvent instance will be copied verbatim, which can
     ...
     * @returns {SendingNetworkEvent} A snapshot of this event.
     */
    toSnapshot(): SendingNetworkEvent {
        const ev = new SendingNetworkEvent(JSON.parse(JSON.stringify(this.event)));
     ...
     * @param {SendingNetworkEvent} otherEvent The other event to check against.
     * @returns {boolean} True if the events are the same, false otherwise.
     */
    isEquivalentTo(otherEvent: SendingNetworkEvent): boolean {
 ...
 *  http://sending.network/speculator/spec/HEAD/client_server/latest.html#redactions
 *
 * Also:
 ...
 *
 * @event module:models/event.SendingNetworkEvent#"Event.decrypted"
 *
 * @param {module:models/event.SendingNetworkEvent} event
 *    The sendingnetwork event which has been decrypted
