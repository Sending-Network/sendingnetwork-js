 ...
 * @param {boolean} opts.event True to make a SendingNetworkEvent.
...
export function mkEvent(opts) {
    ...
    return opts.event ? new SendingNetworkEvent(event) : event;
}

/**
 * Create an m.presence event.
 * @param {Object} opts Values for the presence.
 * @return {Object|SendingNetworkEvent} The event
 */
...
HttpResponse.prototype.request = function(
    ...
    const logLine = (
        "SendingNetworkClient[UT] RECV " + method + " " + path + "  " +
    ...
    if (!next) { // no more things to return
        if (method === "GET" && path === "/sync" && this.ignoreUnhandledSync) {
            logger.log("SendingNetworkClient[UT] Ignoring.");
            return new Promise(() => {});
        }
    ...
    if (next.path === path && next.method === method) {
        logger.log(
            "SendingNetworkClient[UT] Matched. Returning " +
...
HttpResponse.KEEP_ALIVE_PATH = "/_api/client/versions";
