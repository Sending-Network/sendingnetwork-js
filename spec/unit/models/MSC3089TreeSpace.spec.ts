...
import { SendingNetworkClient } from "../../../src";
...
describe("MSC3089TreeSpace", () => {
    let client: SendingNetworkClient;
    let room: any;
    ...
    function makePowerLevels(content: any) {
        powerLevels = new SendingNetworkEvent({
    ...
    it('should not retry invite permission errors', async () => {
        ...
        const fn = jest.fn().mockImplementation((inviteRoomId: string, userId: string) => {
            expect(inviteRoomId).toEqual(roomId);
            expect(userId).toEqual(target);
            return Promise.reject(new SendingNetworkError({ errcode: "M_FORBIDDEN", error: "Sample Failure" }));
        });
        client.invite = fn;
        try {
            await tree.invite(target, false, false);
    ...
    describe('get and set order', () => {
        ...
        // @ts-ignore - "SendingNetworkEvent is a value but used as a type", which is true but not important
    ...
    it('should support getting files', () => {
        const fileEventId = "$file";
        const fileEvent = { forTest: true }; // SendingNetworkEvent mock
        room.currentState = {
