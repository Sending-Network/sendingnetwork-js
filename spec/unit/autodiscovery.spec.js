...
import MockHttpBackend from "sendingnetwork-mock-request";
...
describe("AutoDiscovery", function() {
    ...
    it("should return PROMPT when .well-known 404s", function() {
        httpBackend.when("GET", "/.well-known/api/client").respond(404, {});
        return Promise.all([
    ...
    it("should return FAIL_PROMPT when .well-known returns a 500 error", function() {
        httpBackend.when("GET", "/.well-known/api/client").respond(500, {});
        return Promise.all([
            httpBackend.flushAllExpected(),
            AutoDiscovery.findClientConfig("example.org").then((conf) => {
                const expected = {
    ...
    it("should return FAIL_PROMPT when .well-known returns a 400 error", function() {
        httpBackend.when("GET", "/.well-known/api/client").respond(400, {});
        return Promise.all([
            httpBackend.flushAllExpected(),
            AutoDiscovery.findClientConfig("example.org").then((conf) => {
                const expected = {
    ...
    it("should return FAIL_PROMPT when .well-known returns an empty body", function() {
        httpBackend.when("GET", "/.well-known/api/client").respond(200, "");
        return Promise.all([
            httpBackend.flushAllExpected(),
            AutoDiscovery.findClientConfig("example.org").then((conf) => {
                const expected = {
    ...
    it("should return FAIL_PROMPT when .well-known returns not-JSON", function() {
        httpBackend.when("GET", "/.well-known/api/client").respond(200, "abc");
        return Promise.all([
            httpBackend.flushAllExpected(),
            AutoDiscovery.findClientConfig("example.org").then((conf) => {
                const expected = {
    ...
    it("should return FAIL_ERROR when .well-known has an invalid base_url for " +
        "m.node (disallowed scheme)", function() {
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
    ...
    it("should return FAIL_ERROR when .well-known has an invalid base_url for " +
        "m.node (verification failure: 404)", function() {
        httpBackend.when("GET", "/_api/client/versions").respond(404, {});
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
        ...
        return Promise.all([
            ...
            AutoDiscovery.findClientConfig("example.org").then((conf) => {
                const expected = {
                    "m.node": {
                        state: "FAIL_ERROR",
                        error: AutoDiscovery.ERROR_INVALID_NODE,
                        base_url: "https://example.org",
                    },
                    "m.identity_server": {
                        state: "PROMPT",
    ...
    it("should return FAIL_ERROR when .well-known has an invalid base_url for " +
        "m.node (verification failure: 500)", function() {
        httpBackend.when("GET", "/_api/client/versions").respond(500, {});
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
                base_url: "https://example.org",
            },
    ...
    it("should return FAIL_ERROR when .well-known has an invalid base_url for " +
        "m.node (verification failure: 200 but wrong content)", function() {
        httpBackend.when("GET", "/_api/client/versions").respond(200, {
            not_sendingnetwork_versions: ["r0.0.1"],
        });
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
                base_url: "https://example.org",
    ...
    it("should return SUCCESS when .well-known has a verifiably accurate base_url for " +
        "m.node", function() {
        httpBackend.when("GET", "/_api/client/versions").check((req) => {
            expect(req.opts.uri).toEqual("https://example.org/_api/client/versions");
        }).respond(200, {
            versions: ["r0.0.1"],
        });
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
    ...
    it("should return SUCCESS with the right node URL", function() {
        httpBackend.when("GET", "/_api/client/versions").check((req) => {
            expect(req.opts.uri)
                .toEqual("https://chat.example.org/_api/client/versions");
        }).respond(200, {
            versions: ["r0.0.1"],
        });
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
    ...
    it("should return SUCCESS / FAIL_PROMPT when the identity server configuration " +
        ...
        httpBackend.when("GET", "/_api/identity/api/v1").respond(404, {});
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
                // Note: we also expect this test to trim the trailing slash
                base_url: "https://chat.example.org/",
    ...
    it("should return SUCCESS / FAIL_PROMPT when the identity server configuration " +
        ...
        httpBackend.when("GET", "/_api/identity/api/v1").respond(500, {});
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
                // Note: we also expect this test to trim the trailing slash
                base_url: "https://chat.example.org/",
    ...
    it("should return SUCCESS when the identity server configuration is " +
        ...
        httpBackend.when("GET", "/_api/identity/api/v1").check((req) => {
            expect(req.opts.uri)
                .toEqual("https://identity.example.org/_api/identity/api/v1");
        }).respond(200, {});
        httpBackend.when("GET", "/.well-known/api/client").respond(200, {
            "m.node": {
                // Note: we also expect this test to trim the trailing slash
