...
describe("MegolmDecryption", function() {
    ...
    describe('receives some keys:', function() {
        ...
        it("can detect replay attacks", function() {
            ...
            const event1 = new SendingNetworkEvent({
                type: 'm.room.encrypted',
                room_id: ROOM_ID,
                content: {
                    algorithm: 'm.megolm.v1.aes-sha2',
            ...
            return megolmDecryption.decryptEvent(event1).then((res) => {
                const event2 = new SendingNetworkEvent({
                    type: 'm.room.encrypted',
                    room_id: ROOM_ID,
                    content: {
                        algorithm: 'm.megolm.v1.aes-sha2',
    ...
    it("notifies devices that have been blocked", async function() {
        ...
        aliceClient.sendToDevice = async (msgtype, contentMap) => {
            run = true;
            expect(msgtype).toBe("org.sendingnetwork.room_key.withheld");
    ...
    it("throws an error describing why it doesn't have a key", async function() {
        ...
        aliceClient.crypto.onToDeviceEvent(new SendingNetworkEvent({
            type: "org.sendingnetwork.room_key.withheld",
        ...
        await expect(aliceClient.crypto.decryptEvent(new SendingNetworkEvent({
            type: "m.room.encrypted",
            sender: "@bob:example.com",
            event_id: "$event",
            room_id: roomId,
