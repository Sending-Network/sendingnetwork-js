...
import { SendingNetworkEvent } from "../../../../src/models/event";
...
function makeMockClient(userId, deviceId) {
    ...
    return {
        ...
        sendEvent(roomId, type, content) {
            ...
            events.push(new SendingNetworkEvent({
                sender: userId,
                event_id: eventId,
                room_id: roomId,
                type,
        ...
        sendToDevice(type, msgMap) {
            for (const userId of Object.keys(msgMap)) {
                ...
                for (const deviceId of Object.keys(deviceMap)) {
                    ...
                    const event = new SendingNetworkEvent({ content, type });
...
function makeRemoteEcho(event) {
    return new SendingNetworkEvent(Object.assign({}, event.event, {
        unsigned: {
            transaction_id: "abc",
        },
    }));
...
describe("verification request unit tests", function() {
    ...
    it("transition from UNSENT to DONE through happy path", async function() {
        const alice = makeMockClient("@alice:sendingnetwork.tld", "device1");
        const bob = makeMockClient("@bob:sendingnetwork.tld", "device1");
    ...
    it("other client accepting request puts it in observeOnly mode", async function() {
        const alice = makeMockClient("@alice:sendingnetwork.tld", "device1");
        const bob1 = makeMockClient("@bob:sendingnetwork.tld", "device1");
        const bob2 = makeMockClient("@bob:sendingnetwork.tld", "device2");
        const aliceRequest = new VerificationRequest(
    ...
    it("verify own device with to_device messages", async function() {
        ...
        const to = { userId: "@bob:sendingnetwork.tld", deviceId: "device2" };
        ...
        const [doneEvent1] = bob2.popDeviceEvents("@bob:sendingnetwork.tld", "device1");
        expect(doneEvent1.getType()).toBe(DONE_TYPE);
        await bob1Request.channel.handleEvent(doneEvent1, bob1Request, true);
        const [doneEvent2] = bob1.popDeviceEvents("@bob:sendingnetwork.tld", "device2");
