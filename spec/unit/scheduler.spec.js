...
import { SendingNetworkError } from "../../src/http-api";
import { SendingNetworkScheduler } from "../../src/scheduler";
import * as utils from "../test-utils";

jest.useFakeTimers();

describe("SendingNetworkScheduler", function() {
    let scheduler;
    let retryFn;
    let queueFn;
    let deferred;
    ...
    beforeEach(function() {
        scheduler = new SendingNetworkScheduler(function(ev, attempts, err) {
            if (retryFn) {
    ...
    describe("QUEUE_MESSAGES", function() {
        it("should queue m.room.message events only", function() {
            expect(SendingNetworkScheduler.QUEUE_MESSAGES(eventA)).toEqual("message");
            expect(SendingNetworkScheduler.QUEUE_MESSAGES(
                utils.mkMembership({
    ...
    describe("RETRY_BACKOFF_RATELIMIT", function() {
        it("should wait at least the time given on M_LIMIT_EXCEEDED", function() {
            const res = SendingNetworkScheduler.RETRY_BACKOFF_RATELIMIT(
                eventA, 1, new SendingNetworkError({
        ...
        it("should do exponential backoff", function() {
            expect(SendingNetworkScheduler.RETRY_BACKOFF_RATELIMIT(
                eventA, 1, {},
            )).toEqual(2000);
            expect(SendingNetworkScheduler.RETRY_BACKOFF_RATELIMIT(
                eventA, 2, {},
