#!/bin/bash
#
# Script to perform a release of sendingnetwork-js-sdk and downstream projects.
#
# Requires:
#   github-changelog-generator; install via:
#     pip install git+https://github.com/sending-network/github-changelog-generator.git
...
#
# Note: this script is also used to release sendingnetwork-react-sdk and element-web.
...
expected_npm_user="sendingNetworkdotorg"
...
# `sendingnetwork_lib_main` and `sendingnetwork_lib_typings` fields to `main` and `typings` (if
...
do
    lib_value=$(jq -r ".sendingnetwork_lib_$i" package.json)
    if [ "$lib_value" != "null" ]; then
        jq ".$i = .sendingnetwork_lib_$i" package.json > package.json.new && mv package.json.new package.json
    fi
done
...
if [ -z "$skip_jsdoc" ]; then
    ...
    git pull
    cp -a ".jsdoc/sendingnetwork-js-sdk/$release" .
...
if [ $(git branch -lr | grep origin/develop -c) -ge 1 ]; then
    ...
    do
        ...
        if [ "$(jq -r ".sendingnetwork_lib_$i" package.json)" != "null" ]; then
            ...
            src_value=$(jq -r ".sendingnetwork_src_$i" package.json)
            if [ "$src_value" != "null" ]; then
                jq ".$i = .sendingnetwork_src_$i" package.json > package.json.new && mv package.json.new package.json
            else
